...
from .models import Movie, Review, db
from . import app
...
@app.route('/movie/<int:id>', methods=['GET', 'POST'])   # Страница фильма
def movie(id):
...
@app.route('/add_movie', methods=['GET', 'POST'])   # Страница добавления фильма
def add_movie():
    form = MovieForm()
    if form.validate_on_submit():
        movie = Movie()
...
@app.route('/delete_review/<int:id>')   # Удаление отзыва
